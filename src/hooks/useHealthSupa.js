...
export function useHealthSupa() {
  ...
  useEffect(() => {
    ...
    if (supabase) {
      const channel = supabase
        .channel("health-intel")
        .on("postgres_changes", { event: "*", schema: "public", table: "health_intelligence" }, () => refresh())
        .subscribe();
  ...
  const recompute = useCallback(async () => {
    ...
    try {
      ...
      console.error("[useHealthSupa] recompute failed:", e);
    }
  }, [refresh]);
