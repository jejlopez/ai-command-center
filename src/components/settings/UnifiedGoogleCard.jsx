...
export function UnifiedGoogleCard({ status, onLinked }) {
  ...
  return (
    <div className="rounded-2xl border border-jarvis-cyan/25 bg-gradient-to-b from-jarvis-cyan/[0.04] to-transparent p-5 relative overflow-hidden">
      <div className="absolute -top-16 left-1/2 -translate-x-1/2 w-[420px] h-[180px] rounded-full bg-jarvis-cyan/10 blur-[80px] pointer-events-none" />

      <div className="relative flex items-start justify-between gap-3 mb-4">
        <div>
          <div className="label text-jarvis-cyan">One-click setup</div>
        ...
        {allLinked && (
          <span className="chip bg-jarvis-green/10 text-jarvis-green shadow-glow-green shrink-0">
      ...
      {allLinked ? (
        ...
        <button
          ...
          className="relative w-full py-2.5 rounded-xl bg-jarvis-cyan/15 text-jarvis-cyan hover:bg-jarvis-cyan/25 shadow-glow-cyan text-[13px] font-semibold transition flex items-center justify-center gap-2"
        >
        ...
        <div className="relative space-y-3">
          ...
          <a
            ...
            className="w-full py-2 px-3 rounded-lg bg-jarvis-cyan/10 text-jarvis-cyan hover:bg-jarvis-cyan/20 text-[12px] font-semibold transition flex items-center justify-center gap-2"
          >
          ...
          <div className="flex items-center gap-2 text-[10px] text-jarvis-muted bg-black/40 rounded-lg px-3 py-2">
            ...
            <code className="text-jarvis-cyan truncate flex-1">{REDIRECT_URI}</code>
            <button
              onClick={copyRedirect}
              title="Copy"
              className="text-jarvis-muted hover:text-jarvis-cyan shrink-0"
            >
          ...
          <input
            ...
            className="w-full rounded-lg bg-black/40 border border-jarvis-border px-3 py-2 text-xs text-jarvis-ink placeholder:text-jarvis-muted outline-none focus:border-jarvis-cyan/50"
          />
          <input
            type="password"
          ...
          <button
            ...
            className="w-full py-2.5 px-3 rounded-xl bg-jarvis-cyan/15 text-jarvis-cyan hover:bg-jarvis-cyan/25 shadow-glow-cyan text-[13px] font-semibold transition flex items-center justify-center gap-2 disabled:opacity-50"
          >
...
function HelpExpander() {
  ...
  return (
    <div>
      <button
        onClick={() => setOpen((v) => !v)}
        className="text-[11px] text-jarvis-muted hover:text-jarvis-cyan flex items-center gap-1"
      >
        ...
        <ol className="mt-2 text-[11px] text-jarvis-body list-decimal pl-5 space-y-1.5">
          <li>
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer noopener"
              className="text-jarvis-cyan hover:text-jarvis-ink inline-flex items-center gap-1"
            >
